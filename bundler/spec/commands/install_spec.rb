...
RSpec.describe "bundle install with gem sources" do
  describe "the simple case" do
    ...
    it "throws a warning if a gem is added once in Gemfile and also inside a gemspec as a development dependency, with different requirements" do
      ...
      expect(err).to include("A gemspec development dependency (rubocop, ~> 1.36.0) is being overriden by a Gemfile dependency (rubocop, >= 0).")
      expect(err).to include("Please remove either of them, or make sure they both have the same requirement")
