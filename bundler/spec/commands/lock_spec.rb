...
RSpec.describe "bundle lock" do
  ...
  shared_examples_for "only the current platform added" do
    around do |example|
      simulate_platform(specific_local_platform) do
        gemfile <<-G
  ...
  context "when on Linux" do
    let(:specific_local_platform) { linux }

    include_examples "only the current platform added"
  end

  context "when on Windows" do
    let(:specific_local_platform) { x64_mingw }

    include_examples "only the current platform added"
  end

  context "when on MacOS" do
    let(:specific_local_platform) { mac }

    around do |example|
      simulate_platform(specific_local_platform, &example)
    end

    it "locks both Darwin and Linux" do
      gemfile <<-G
        source "#{file_uri_for(repo)}"
        gem "rails"
        gem "weakling"
      ...
      allow(Bundler::SharedHelpers).to receive(:find_gemfile).and_return(bundled_app_gemfile)
      lockfile = Bundler::LockfileParser.new(read_lockfile)
      expect(lockfile.platforms).to match_array([linux, specific_local_platform])
    end
  end
end
